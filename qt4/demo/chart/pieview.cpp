...
QRect PieView::itemRect(const QModelIndex &index) const
{
    ...
    if (model()->data(valueIndex).toDouble() > 0.0) {
        ...
        int listItem = 0; // number of unzero items before current row
        ...
        case 1:
            return viewport()->rect(); // entire rect for column 1 ?
        }
...
QRegion PieView::itemRegion(const QModelIndex &index) const
{
    ...
    if (index.column() != 1) // rect region for labels
...
void PieView::setSelection(const QRect &rect, QItemSelectionModel::SelectionFlags command)
{
    ...
    if (indexes.size() > 0) {
        // search top left and botoom right model index

...
#include "../find_dialog/find_dialog.h"
#include "../gotocell_dialog/gotocell_dialog.h"
#include "../sort_dialog/sort_dialog.h"
...
MainWindow::MainWindow()
{
    ...
    findDialog = 0; // will be set when find() is called for the first time

    // When we refer to qt resources, we use the path prefix :/
...
void MainWindow::find()
{
    ...
    // A call to show() alone is sufficient to make a hidden window visible, on
    // top, and active, but the Find dialog may be invoked when it is already
    // visible. In that case, show() does nothing and we must call raise() and
    // activateWindow() to make the window on top and active.
    findDialog->show();
    findDialog->raise();
...
{
    newAction = new QAction(tr("&New"), this); // specify accelerator and parent
    ...
    exitAction->setShortcut(tr("Ctrl+Q")); // explicit short cut
...
void MainWindow::createMenus()
{
    ...
    // kept a pointer to one of the separators. This will allow us to hide the
    // separator (if there are no recent files) or to show it
...
void MainWindow::createContextMenu()
{
    ...
    // QWidget::contextMenuEvent() function, create a QMenu widget, populate it
    // with the desired actions, and call exec() on it.
}
...
{
    locationLabel = new QLabel(" W999 "); // W999 is largest possible value

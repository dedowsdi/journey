...
public:
  ...
  virtual void attach_shaders(){
    attach(GL_VERTEX_SHADER, "data/shader/debugger.vs.glsl");
    attach(GL_FRAGMENT_SHADER, "data/shader/debugger.fs.glsl");
  };
...
//--------------------------------------------------------------------
void draw_point(const zxd::vec2_vector& vertices, const glm::mat4& mvp, GLfloat size/* = 1*/, const glm::vec4& color/* = glm::vec4(1)*/)
{
  using namespace glm;
  static GLuint vao = -1;
  static GLuint vbo = -1;
  ...
  GLuint new_size = vertices.size() * sizeof(vec2);
  if(buffer_size == new_size)
    glBufferSubData(GL_ARRAY_BUFFER, 0, new_size, value_ptr(vertices[0]));
  else
    glBufferData(GL_ARRAY_BUFFER, new_size, value_ptr(vertices[0]), GL_DYNAMIC_DRAW);
...
//--------------------------------------------------------------------
void draw_line(const glm::vec2& p0, const glm::vec2& p1, const glm::mat4& mvp, GLfloat width/* = 1*/, const glm::vec4& color/* = glm::vec4(1)*/)
{
  using namespace glm;
  static GLuint vao = -1;
  static GLuint vbo = -1;
  ...
  vec2 vertices[2] = {p0, p1};

  glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(vec2)*2, value_ptr(vertices[0]));
...
//--------------------------------------------------------------------
void draw_line(GLenum mode, const zxd::vec2_vector& vertices, const glm::mat4& mvp, GLfloat width/* = 1*/, const glm::vec4& color/* = glm::vec4(1)*/)
{
  using namespace glm;
  static GLuint vao = -1;
  static GLuint vbo = -1;

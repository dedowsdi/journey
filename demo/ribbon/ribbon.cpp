...
osg::Geometry* createRibbon(const osg::Vec3& colorRGB) {
  ...
  // At the beginning, all vertices are placed together at the origin point, but
  // later they will be treated as the two sides of the ribbon. The colors are
  // computed with a sine function to implement a fade-in and fade-out effect
  // while the ribbon is moving
  //
...
public:
  ...
  virtual void operator()(osg::Node* node, osg::NodeVisitor* nv) {
    ...
    if (trans && _ribbon.valid()) {
      const osg::Matrix& matrix = trans->getMatrix();
      osg::Vec3Array* vertices =
        static_cast<osg::Vec3Array*>(_ribbon->getVertexArray());
      osg::Vec3Array* normals =
        static_cast<osg::Vec3Array*>(_ribbon->getNormalArray());

      //read and accept the values of the next 2 vertices
      ...
      //connect the last 2 vertices to transform node
      (*vertices)[g_numPoints - 2] =
        osg::Vec3(0.0f, -g_halfWidth, 0.0f) * matrix;

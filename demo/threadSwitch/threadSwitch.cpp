...
int main(int argc, char* argv[]) {
  ...
  if (arguments.read("--single"))
    th = osgViewer::ViewerBase::SingleThreaded;
  else if (arguments.read("--useContext"))  // extra thread for cull and draw
    th = osgViewer::ViewerBase::ThreadPerContext;
  // osg default thread model, at least one cull thread and one draw thread for
  // each render window
  else if (arguments.read("--useCamera"))
    th = osgViewer::ViewerBase::ThreadPerCamera;

  osgViewer::View* view1 =
    createView(50, 50, 640, 480, createMassiveQuads(10000));
  osgViewer::View* view2 =
    createView(50, 550, 320, 240, createMassiveQuads(5000));
  osgViewer::View* view3 =
    createView(370, 550, 320, 240, createMassiveQuads(5000));

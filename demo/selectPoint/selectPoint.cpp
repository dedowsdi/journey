...
public:
  ...
  virtual void doUserOperations(
    ...
    osg::Vec3Array* selVertices =
      dynamic_cast<osg::Vec3Array*>(_selector->getVertexArray());
    ...
    //Find out all three vertices of the nearest picked triangle, and compute
    //the distance between the intersection point and each of them. If any one
    //of the distances is less than a fixed threshold ( 0.1 ), we will say that
...
protected:
  osg::ref_ptr<osg::Geometry> _selector; //visual selected point
...
int main(int argc, char** argv) {
  ...
  root->addChild(
    selector->createPointSelector());  // Caution: It has bound, too

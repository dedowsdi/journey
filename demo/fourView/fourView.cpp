...
protected:
  double _distance;  // scale factor
  float _offsetX, _offsetY; //mouse offset during dragging
  float _lastDragX, _lastDragY;
};

osgViewer::View* create2DView(int x, int y, int width, int height,
  const osg::Vec3& lookDir, const osg::Vec3& up, osg::GraphicsContext* gc,
  osg::Node* scene) {
  ...
  camera->setViewMatrixAsLookAt(center - lookDir * (radius * 3.0), center, up);
...
int main(int argc, char** argv) {
  ...
  //set view port, this can not be omitted!!!, there is no default viewport if
  //you youse custom gc

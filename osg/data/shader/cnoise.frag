...
float cnoise( float p )
{
    ...
    // Should [-1,1] by convention
    return mix( random( i ), random( i + 1 ), fade( f ) ) * 2.0 - 1.0;
}

// Classic Perlin noise
float cnoise( vec2 P )
{
    // get integer, float part of P and P + vec2(1, 1)
    vec4 Pi = floor( P.xyxy ) + vec4( 0.0, 0.0, 1.0, 1.0 );
    vec4 Pf = fract( P.xyxy ) - vec4( 0.0, 0.0, 1.0, 1.0 );
    Pi = mod289( Pi ); // To avoid truncation effects in permutation

    // integer, float part of g00, g10, g01, g11
    vec4 ix = Pi.xzxz;
    vec4 iy = Pi.yyww;
    vec4 fx = Pf.xzxz;
    vec4 fy = Pf.yyww;
